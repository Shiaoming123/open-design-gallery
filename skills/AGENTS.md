...
skill, do not paste raw frontmatter or descriptions into JavaScript
strings. Generate the `CARDS` entry through a structured serializer
(`JSON.stringify` or equivalent), normalize embedded newlines, and run
the `overview.html` validation steps in `../docs/overview.md` before
calling the catalogue updated.
