...
If you also refresh the root [`overview.html`](../overview.html) catalogue,
generate card data with `JSON.stringify` or an equivalent structured
serializer. Do not paste `DESIGN.md` excerpts, font names, or descriptions
directly into JavaScript strings; raw newlines can break the inline `CARDS`
dataset and leave the page rendering only its static search boxes. Run the
validation steps in [`docs/overview.md`](../docs/overview.md) before treating
the overview as updated.
